<div x-data="{ open: false }" class="relative flex flex-col w-[32px] h-[32px] bg-[#1a2e35] justify-center rounded">
    <button @click="open = !open" class="relative w-[26px] h-[26px]">
        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="26" height="26" viewBox="0 0 300 300">
    ...
    >
        ...
        <div class="p-4 overflow-y-auto h-[calc(100%-56px)]">
            @forelse($notifications as $note)
                <div
                    class="flex items-start p-4 mb-2 rounded-lg cursor-pointer transition hover:bg-gray-100 dark:hover:bg-gray-800"
                    wire:click="markAsRead({{ $note->id }})"
                >
                    <div class="w-2.5 h-2.5 rounded-full mt-1 mr-3 {{ $note->read ? 'bg-transparent' : 'bg-orange-500' }}"></div>
                    <div>
