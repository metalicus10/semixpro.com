...
<div class="p-2 md:p-4 bg-white dark:bg-gray-900 rounded-lg overflow-hidden">
    ...
    <div class="flex flex-row space-x-4">
        <div x-data="{
                ...
                selectedWarehouseId: @entangle('selectedWarehouseId'),
                ...
                init() {
                    let savedId = Number(localStorage.getItem('activeWarehouseId'));
                    if (savedId) {
                        this.selectedWarehouseId = savedId;
                    }
                    this.loadPartsForWarehouse(this.selectedWarehouseId);
                    ...
                    $watch('selectedWarehouseId', id => {
                        ...
                        this.loadPartsForWarehouse(id);
                    });
                },
                selectWarehouseTab(id) {
                    if (this.isPartsLoading) return;
                    if (this.selectedWarehouseId === id) { return; }
                    this.isPartsLoading = true;
                    if(id){
                        if (this.partsCache[id]) {
                            this.parts = this.partsCache[id];
                        ...
                        $wire.selectWarehouse(id).then(data => {
                            this.parts = data.parts || [];
                            this.partsCache[id] = this.parts;
                            this.isPartsLoading = false;
                        });
                ...
                updateActiveTab(tabId){
                    this.selectedWarehouseId = tabId;
                    search = '';
                },
                ...
                loadPartsForWarehouse(id = null) {
                    savedId = id || localStorage.getItem('activeWarehouseId');
                    if (!this.warehouses || !this.warehouses.length) return;
                    if (this.selectedWarehouseId === savedId && !id) return;

                    let found = this.warehouses.find(w => w.id == savedId);
                    if (savedId && found) {
                        this.selectedWarehouseId = Number(savedId);
                    } else {
                        this.selectedWarehouseId = this.warehouses[0].id;
                        localStorage.setItem('activeWarehouseId', this.selectedWarehouseId);
                    }
                    this.isPartsLoading = true;
                    if (this.partsCache && this.partsCache[this.selectedWarehouseId]) {
                        this.parts = this.partsCache[this.selectedWarehouseId];
                    } else {
                        this.parts = [];
                    }
                    $wire.selectWarehouse(this.selectedWarehouseId).then(data => {
                        this.parts = data.parts || [];
                        this.selectedWarehouseId  = id;
                        // Caching parts
                        if (!this.partsCache) this.partsCache = {};
                        this.partsCache[this.selectedWarehouseId] = this.parts;

                        this.$nextTick(() => { this.isPartsLoading = false });
                    });
                },
            }"
                ...
                <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                    ...
                    <div class="flex flex-row justify-between items-center">
                        ...
                        <select x-model="selectedBrand" id="brand"
                                class="w-28 ml-2 p-2 dark:text-gray-400 border border-gray-300 text-gray-50 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        <template x-for="warehouse in warehouses" :key="warehouse.id">
                            <li class="shrink-0 cursor-pointer rounded-lg font-semibold hover:scale-105 transition-all duration-200" :id="warehouse.id"
                                ...
                                <div x-data="{ isEditing: false }">
                                    ...
                                    <a href="#"
                                       @click.prevent.debounce.500ms="if (!isEditing) { selectedWarehouseId = warehouse.id, updateActiveTab(warehouse.id) }"
                                       x-show="editingTabId !== warehouse.id"
                                       @dblclick="isEditing = true; startEdit(warehouse.id, 'warehouse.name')"
                                       :class="selectedWarehouseId == warehouse.id ? 'text-orange-500 bg-[#b13a00] dark:bg-green-500 dark:text-white' : 'hover:text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 dark:hover:text-gray-300'"
                                    ...
                                    >
                                        ...
                                        <svg x-show="isPartsLoading && selectedWarehouseId === warehouse.id"
                                            class="animate-spin inline-block ml-1 text-current"
            ...
            <div class="bg-white dark:bg-gray-800 shadow-md rounded-md">
                @if (isset($selectedWarehouseId) && isset($parts))
                    ...
                    <h2 class="text-gray-700 dark:text-gray-400 uppercase text-lg font-semibold p-3">Запчасти
                        склада <span class="dark:text-orange-500" x-text="getWarehouseName(selectedWarehouseId)"></span></h2>
                    <div id="parts-table"
                        ...
                        <div class="flex flex-col space-y-2 md:space-y-0 dark:bg-gray-900" :class="{ 'opacity-50 pointer-events-none': isPartsLoading }">
                            ...
                            <template x-if="isPartsLoading">
                                <div class="w-full inset-0 z-10 bg-black/10 flex flex-col justify-center pointer-events-none animate-pulse transition-opacity">
                                    <template x-for="n in 5" :key="n">
                                        <div class="flex items-center space-y-2 gap-10 py-2">
                                            <div class="px-4 py-2 w-4 h-4 bg-gray-300/40 rounded"></div>
                                            <div class="w-[100px] px-4 py-2 bg-gray-300/50 rounded"></div>
                                            <div class="flex-1 px-4 py-2 bg-gray-200/60 rounded"></div>
                                            <div class="flex-1 px-4 py-2 bg-gray-200/60 rounded"></div>
                                            <div class="flex-1 px-4 py-2 bg-gray-200/60 rounded"></div>
                                            <div class="flex-1 px-4 py-2 bg-gray-200/60 rounded"></div>
                                            <div class="w-[150px] px-4 py-2 bg-gray-200/60 rounded"></div>
                                            <div class="flex-1 flex px-4 py-2 bg-gray-200/60 rounded"></div>
                                            <div class="w-[150px] px-4 py-2 bg-gray-200/60 rounded"></div>
                                            <div class="w-[150px] px-4 py-2 bg-gray-200/60 rounded"></div>
                                        </div>
                                    </template>
                            ...
                            <div x-show="!isPartsLoading && filteredParts().length === 0" x-transition class="text-gray-400 text-center mt-4">
                                Ничего не нашлось
                            </div>
                        </div>
                    </div>
