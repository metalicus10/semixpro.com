<div x-data="createJobModal()" x-init="init()" x-show="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
    <div class="bg-white rounded-lg shadow-lg w-full max-w-6xl p-6 overflow-y-auto max-h-[95vh]">
        <div class="flex justify-between items-center border-b pb-4 mb-6">
            ...
            <button @click="open = false" class="text-gray-500 hover:text-red-500 text-2xl">&times;</button>
        </div>

        <div class="flex flex-col lg:flex-row gap-6">
            <!-- Left Column -->
            <div class="w-full lg:w-1/3 space-y-6">
                <!-- Customer Search (вставляется из шаблона) -->
                <!-- ... (см. основной layout) -->
                ...
                <div class="border p-4 rounded space-y-4">
                    ...
                    <div class="flex flex-col gap-2">
                        <div>
                            ...
                            <input type="datetime-local" x-model="form.schedule_from" class="w-full border rounded px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="text-xs text-gray-500">To</label>
                            <input type="datetime-local" x-model="form.schedule_to" class="w-full border rounded px-3 py-2 text-sm">
                        </div>
                        <div class="text-xs text-gray-500">Timezone: EDT</div>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-500">Dispatch</label>
                        <input type="text" x-model="form.dispatch" placeholder="Dispatch by name or tag" class="w-full border rounded px-3 py-2 text-sm">
                    </div>
            ...
            <div class="w-full lg:w-2/3 space-y-6">
                    ...
                    <template x-for="(item, index) in form.items" :key="index">
                        <div class="mb-4 space-y-2 border-b pb-2">
                            <div class="flex flex-wrap gap-2">
                                <input x-model="item.name" type="text" placeholder="Item name" class="flex-1 border rounded px-3 py-2 text-sm">
                                <input x-model="item.qty" type="number" step="0.01" min="0" class="w-20 border rounded px-2 py-1 text-sm" placeholder="Qty">
                                <input x-model="item.unit_price" type="number" step="0.01" min="0" class="w-24 border rounded px-2 py-1 text-sm" placeholder="Unit price">
                                <input x-model="item.unit_cost" type="number" step="0.01" min="0" class="w-24 border rounded px-2 py-1 text-sm" placeholder="Unit cost">
                                <label class="flex items-center gap-1 text-sm">
                                    <input type="checkbox" x-model="item.tax" class="form-checkbox">
                                    Tax
                                </label>
                            </div>
                            <textarea x-model="item.description" placeholder="Description (optional)" class="w-full border rounded px-3 py-2 text-sm"></textarea>
                        </div>
                    </template>
                ...
                <div>
                    ...
                    <template x-if="form.message !== null">
                        <textarea x-model="form.message" class="w-full border rounded mt-2 px-3 py-2 text-sm" placeholder="Add a message..."></textarea>
                    </template>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end mt-6">
            <button @click="$wire.call('saveJob', form); open = false" class="bg-blue-600 text-white px-6 py-2 rounded">Save job</button>
        </div>
    </div>
</div>
